...
def print_status(entry: dict, startTime=None) -> None:
    ...
    for evaluate_id, _ in entry.items():
        scoreboard = (
            str(entry[evaluate_id]["wins"])
            + ":"
            + str(entry[evaluate_id]["losses"])
            + ":"
            + str(entry[evaluate_id]["draws"])
        )
        print(evaluate_id, scoreboard)
        print(
            "Wins:",
            entry[evaluate_id]["wins"],
            "(" + str(entry[evaluate_id]["win_ratio"]) + "%",
            "-",
            entry[evaluate_id]["as_black"]["wins"],
            "as black,",
            entry[evaluate_id]["as_white"]["wins"],
            "as white; FMA Ratio: ",
            entry[evaluate_id]["first_mover_advantage"]["win_ratio"],
            ")",
        )
        print(
            "Losses:",
            entry[evaluate_id]["losses"],
            "(" + str(entry[evaluate_id]["lose_ratio"]) + "%",
            "-",
            entry[evaluate_id]["as_black"]["losses"],
            "as black,",
            entry[evaluate_id]["as_white"]["losses"],
            "as white; FMA Ratio: ",
            entry[evaluate_id]["first_mover_advantage"]["lose_ratio"],
            ")",
        )
        print(
            "Draws:",
            entry[evaluate_id]["draws"],
            "(" + str(entry[evaluate_id]["draw_ratio"]) + "%",
            "-",
            entry[evaluate_id]["as_black"]["draws"],
            "as black,",
            entry[evaluate_id]["as_white"]["draws"],
            "as white; FMA Ratio: ",
            entry[evaluate_id]["first_mover_advantage"]["draw_ratio"],
            ")",
        )
        print(
            "KD Ratio:",
            entry[evaluate_id]["kd_ratio"],
            "- Success Ratio:",
            entry[evaluate_id]["success_ratio"],
        )
        print()
        ...
        for x in games:
            ...
            for j in range(0, 2):
                ...
                with multiprocessing.Pool(processes=cores) as pool:
                    scores = pool.map(game_worker, gamePool)
            ...
            entry[evaluate_id]["win_ratio"] = (
                entry[evaluate_id]["wins"] / numberOfGames * 100
            )
            entry[evaluate_id]["lose_ratio"] = (
                entry[evaluate_id]["losses"] / numberOfGames * 100
            )
            entry[evaluate_id]["draw_ratio"] = (
                entry[evaluate_id]["draws"] / numberOfGames * 100
            )
            try:
                entry[evaluate_id]["kd_ratio"] = (
                    entry[evaluate_id]["win_ratio"] / entry[evaluate_id]["lose_ratio"]
                )
            except:
...
if __name__ == "__main__":
    ...
    filename = "evaluations " + str(currentTime)
    if cores > 64:
        evaluate(games, 256, filename)
    else:
        evaluate(games, 2, filename)
