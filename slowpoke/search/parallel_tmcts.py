...
class SharedBatchAccumulator:
    ...
    def get_cached_value(self, pos_key: bytes) -> Optional[float]:
        ...
        Lock-free read — atomic dict lookup under CPython GIL.
        Used by _tree_search_batch for tree reuse across turns.
        """
...
class ParallelTMCTS:
    ...
    def __init__(
        ...
        self.accumulator = SharedBatchAccumulator(
            batch_size=batch_size, auto_flush=False
        )

        # Neural network reference
        self.nn = getattr(evaluator, "nn", None)
        self.use_mlx = (
            hasattr(evaluator, "nn")
            and evaluator.nn is not None
            and getattr(evaluator.nn, "_use_mlx", False)
        )
    ...
    def _decide_impl(self, B, colour: int) -> Any:
        ...
        if (
            self.progressive_narrowing
            and self.use_mlx
            and len(moves) > self.progressive_narrowing_k
        ):
            ...
            if scored:
                ...
                top_k = sorted(range(len(noisy)), key=lambda i: noisy[i], reverse=True)[
                    : self.progressive_narrowing_k
                ]
                narrowed_moves = [scored[i][0] for i in top_k]
                if self.debug:
                    print(
                        f"[ParallelTMCTS] Narrowed {len(moves)} -> {len(narrowed_moves)} root moves"
                    )

        random_rounds = self.base_round * ply if ply > 0 else 1

        # Track move statistics across all instances
        move_stats: Dict[Any, Dict[str, float]] = {
            m: {"wins": 0.0, "plays": 0} for m in narrowed_moves
        }
    ...
    def _evaluate_root_moves(self, B, moves, colour):
        ...
        for move in moves:
            B.push_move(move)
            is_over = self._is_over(B, colour)
            if is_over[0]:
                move_info.append((move, float(is_over[1])))
            else:
    ...
    def _run_instance(
        ...
        for _ in range(rounds):
            ...
            result = self._tree_search_batch(B, ply, colour, rng)
    ...
    def _tree_search_batch(self, B, ply: int, colour: int, rng):
        ...
        if moves:
            ...
            result = self._tree_search_batch(B, ply - 1, colour, rng)
            B.pop_move()
...
def benchmark_parallel_tmcts(
    B,
    colour: int,
    ply: int,
    evaluator,
    num_parallel: int = 4,
    rounds_per_instance: int = 100,
    seed: int = 42,
):
    ...
    single.base_round = rounds_per_instance
    ...
    parallel = ParallelTMCTS(
        ply, evaluator, num_parallel=num_parallel, debug=True, seed=seed
    )
    parallel.base_round = rounds_per_instance
    ...
    print("Parallel with same seed: deterministic (see test_deterministic_with_seed)")
