...
def create_bot_with_parallel_tmcts(ply: int, nn, num_parallel: int = 4, 
                                    use_mlx: bool = True, debug: bool = False, 
    ...
    bot = Slowpoke(plyDepth=ply, layers=nn.layers if hasattr(nn, 'layers') else [91, 40, 10, 1],
                   weights=[], isminimax=False, debug=debug, use_mlx=use_mlx)
    ...
    bot.decisionFunction = parallel_mcts
...
class HybridTournamentConfig:
    ...
    def get_tournament_settings(self) -> dict:
        ...
        return {
            'processors': self.num_game_processes,
            'plyDepth': self.ply
        }
    ...
    def print_config(self):
        ...
        print(f"Hybrid Tournament Configuration:")
...
def run_hybrid_tournament(population_size: int = 15, generations: int = 10,
                          ply: int = 4, num_cores: Optional[int] = None):
    """
    ...
    for player_id in t.population.currentPopulation:
