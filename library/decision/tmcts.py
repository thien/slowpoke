...
class TMCTS:
    ...
    def __init__(
        self, ply: int, evaluator: Any, debug: bool = False, batch_size: int = 512
    ) -> None:
        self.ply = ply
    ...
    def _evaluate_moves_batch(
        self, B: Any, moves: List[int], colour: int
    ...
    def random_ts(self, B: Any, ply: int, colour: int, printDebug: bool = False) -> int:
        ...
        if len(moves) == 1:
            ...
            if ply > 0:
                random_rounds = self.base_round * ply
            ...
            for i in range(random_rounds):
                ...
                if self.use_mlx:
                    result = self.tree_search_batch(B, ply, colour)
            ...
            for m in self.movesets:
                ...
                chance = 0
                if (self.movesets[m]["plays"] > 0) and (
                    self.movesets[m]["chances"] > 0
                ):
                ...
                if chance > bestChance:
                    ...
                    if printDebug:
                        print(
                            f"Move {m}: chance={chance}, wins={self.movesets[m]['chances']}, plays={self.movesets[m]['plays']}  *"
                        )
                else:
                    if printDebug:
    ...
    def tree_search(self, B: Any, ply: int, colour: int) -> float:
            ...
            if ply < 1:
                ...
                if is_over[0]:
                    ...
                    if len(moves) > 0:
                        ...
                        result = self.tree_search(B, ply - 1, colour)
                        B.pop_move()
                    else:
                        result = 0
    ...
    def tree_search_batch(self, B: Any, ply: int, colour: int) -> Union[int, float]:
        ...
        if len(moves) > 0:
            move = random.choice(moves)
            B.push_move(move)
            result = self.tree_search_batch(B, ply - 1, colour)
            B.pop_move()
            B.pop_move()  # Pop enemy move
        else:
            B.pop_move()  # Pop enemy move
    ...
    def flush_batch(self) -> np.ndarray:
        ...
        if (
            ...
            and hasattr(self.nn, "compute_batch_mlx")
        ):
            ...
            if callable(self.evaluator):
                results = np.array(
                    [self.evaluator(None, None) for _ in position_arrays],
                    dtype=np.float32,
                )
            else:
                results = np.array(
                    [
                        self.evaluator.evaluate_board(None, None)
                        for _ in position_arrays
                    ],
                    dtype=np.float32,
                )
