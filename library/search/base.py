...
class MCTSBase:
    ...
    def _extract_position(self, B: Any, colour: int) -> np.ndarray:
        ...
        board_status = B.get_board_pos_weighted(colour, {
            "Black": 1, "White": -1, "empty": 0, "blackKing": 1.5, "whiteKing": -1.5,
        })
        if self.nn is not None and hasattr(self.nn, "layer_size") and self.nn.layer_size[0] == 91:
            board_status = self.nn.subsquares(board_status)
        return np.asarray(board_status, dtype=np.float32)

    @staticmethod
    def _ucb1_score(wins: float, visits: int, total_visits: int, c: float = 1.4) -> float:
