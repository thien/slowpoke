...
class TestParallelBenchmark(unittest.TestCase):
    ...
    def test_benchmark_single_vs_parallel(self):
        ...
        # Parallel instance
        parallel = ParallelTMCTS(ply=2, evaluator=self.evaluator, num_parallel=4, debug=False)
        parallel.baseRound = rounds
        ...
        if time_single/time_parallel < 1.0:
            print(f"Note: Parallel is SLOWER with simple evaluators due to threading overhead.")
        print(f"Real speedup (3-7x) comes from GPU batch evaluation with neural networks.")
    ...
    def test_parallel_scaling(self):
        ...
        for num_parallel in [1, 2, 4, 8]:
            B = self.B.copy()
            parallel = ParallelTMCTS(ply=2, evaluator=self.evaluator, num_parallel=num_parallel, debug=False, seed=42)
            parallel.baseRound = rounds
    ...
    def test_deep_ply_scaling(self):
        ...
        for num_parallel in [1, 2, 4, 8]:
            B = self.B.copy()
            parallel = ParallelTMCTS(ply=6, evaluator=self.evaluator, num_parallel=num_parallel, debug=False, seed=42)
            parallel.baseRound = rounds
...
class TestParallelCorrectness(unittest.TestCase):
    ...
    def test_deterministic_with_seed(self):
        ...
        # Parallel with seed
        B1 = self.B.copy()
        parallel = ParallelTMCTS(ply=2, evaluator=self.evaluator, num_parallel=4, debug=False, seed=seed)
        parallel.baseRound = rounds
        move_parallel = parallel.decide(B1, Black)
        
        # Run again with same seed - should get same move
        B2 = self.B.copy()
        parallel2 = ParallelTMCTS(ply=2, evaluator=self.evaluator, num_parallel=4, debug=False, seed=seed)
        parallel2.baseRound = rounds
        move_parallel2 = parallel2.decide(B2, Black)
        
        self.assertEqual(move_parallel, move_parallel2, "Same seed should produce same move")
