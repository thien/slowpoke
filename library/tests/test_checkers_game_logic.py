...
unusedBits = checkers.unusedBits
...
class TestCheckerBoardInitialization(unittest.TestCase):
    ...
    def test_initial_turn_count_is_one(self):
        ...
        # Note: updateState() is called in __init__ and increments turnCount to 1
        self.assertEqual(self.B.turnCount, 1)
    ...
    def test_copy_creates_independent_board(self):
        ...
        turn_after_move = B1.turnCount
        
        B2 = B1.copy()
        # Note: copy() doesn't preserve turnCount (it's not in copy method)
        # but copy() should have valid board state
        self.assertIsInstance(B2.turnCount, int)
        
        # Both boards should have the same pieces
        self.assertEqual(len(B1.blackPieces), len(B2.blackPieces))
        self.assertEqual(len(B1.whitePieces), len(B2.whitePieces))
...
class TestGameLogic(unittest.TestCase):
    ...
    def test_single_move_increments_turn(self):
        ...
        B = checkers.CheckerBoard()
        initial_turn = B.turnCount
        moves = B.get_moves()
        if moves:
            B.make_move(moves[0])
            self.assertEqual(B.turnCount, initial_turn + 1)
    ...
    def test_jumps_reset_no_eat_count(self):
        ...
        for _ in range(10):
            ...
            if jump_moves:
                ...
                self.assertEqual(B.noEatCount, 0)
                return
            elif moves:
                B.make_move(moves[0])
    ...
    def test_regular_move_increments_no_eat(self):
        ...
        for m in moves:
            if m > 0:  # Regular move
                B.make_move(m)
                self.assertEqual(B.noEatCount, 1)
                return
...
class TestMinimaxImplementation(unittest.TestCase):
    ...
    def test_alphabeta_pruning_basic(self):
...
class TestNeuralNetwork(unittest.TestCase):
    ...
    def test_load_coefficients(self):
        ...
        self.assertEqual(len(nn2.getAllCoefficents()), len(coeffs))
    ...
    def test_load_invalid_coefficient_count_raises(self):
        ...
        with self.assertRaises(ValueError):
            nn.loadCoefficents(wrong_coeffs)
...
class TestGameTermination(unittest.TestCase):
    ...
    def test_no_eat_limit_draw(self):
        ...
        self.assertTrue(B.noEatCount == 50 or B.is_over())
...
class TestEvaluatorIntegration(unittest.TestCase):
    ...
    def test_evaluator_returns_win_for_winning_state(self):
        ...
        B = checkers.CheckerBoard()
        evaluator = lambda b, c: 1.0 if b.winner == c else -1.0 if b.winner != empty else 0
...
class TestMinimaxCorrectness(unittest.TestCase):
    ...
    def test_minimax_terminal_state_with_no_moves(self):
        ...
        B = checkers.CheckerBoard()
        # Create a terminal state by setting noEatCount to the limit
        B.noEatCount = 50  # boringNoEatLimit - causes a draw
        
        mm = minimax.MiniMax(ply=3, evaluator=lambda b, c: 0.5)
        # Call is_over to set the winner
        B.is_over()
        mm.counter = 0
        score = mm.alphabeta(B, 0, float('-inf'), float('inf'), Black, True)
    ...
    def test_alphabeta_respects_alpha_beta_bounds(self):
        ...
        alpha, beta = -0.5, 0.5
        score = mm.alphabeta(B, 2, alpha, beta, Black, True)
