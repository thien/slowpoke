...
class Statistics:
    ...
    def parseLeaderboards(self) -> list:
        ...
        stats = {"persistent": [], "elitism": [], "mutation": [], "crossover": []}
        ...
        colors = [
            cols.get(x, "#888888") for x in xlabels
  ...
  """
    ...
    def timeStatsPerGeneration(self) -> None:
        ...
        for i in range(len(self.statistics)):
            ...
            means.append(mean)
            medians.append(
                max(set(times["gameDurations"]), key=times["gameDurations"].count)
            )
            sx.append(times)
  ...
  """
    ...
    def getLearningRate(self) -> None:
        ...
        for i in range(len(self.statistics)):
            for j in self.statistics[i]["stats"]:
                ...
                if j[0] == "Prev. Champ Point Range":
                    champ_range.append(ast.literal_eval(j[1]))
        ...
        for i in range(len(champ_range)):
            li = champ_range[i]
  ...
  """
    ...
    def createWDLChart(self, opp_names: list, data, title: str = None) -> None:
        ...
        totals = [
            i + j + k + l + m + n for i, j, k, l, m, n in zip(ww, dw, lw, wb, db, lb)
        ]
        ...
        # Create green Bars
        plt.bar(
            r, wb, color=colours["win_black"], edgecolor="white", label="Win (Black)"
        )
        plt.bar(
            r,
            ww,
            bottom=wb,
            color=colours["win_white"],
            edgecolor="white",
            label="Win (White)",
        )
        # Create orange Bars
        plt.bar(
            r,
            db,
            bottom=[i + j for i, j in zip(wb, ww)],
            color=colours["draw_black"],
            edgecolor="white",
            label="Draw (Black)",
        )
        plt.bar(
            r,
            dw,
            bottom=[i + j + k for i, j, k in zip(wb, ww, db)],
            color=colours["draw_white"],
            edgecolor="white",
            label="Draw (White)",
        )
        # Create blue Bars
        plt.bar(
            r,
            lb,
            bottom=[i + j + k + l for i, j, k, l in zip(wb, ww, db, dw)],
            color=colours["lose_black"],
            edgecolor="white",
            label="Loss (Black)",
        )
        plt.bar(
            r,
            lw,
            bottom=[i + j + k + l + m for i, j, k, l, m in zip(wb, ww, db, dw, lb)],
            color=colours["lose_white"],
            edgecolor="white",
            label="Loss (White)",
        )
...
def measure_batch_timings(stats) -> None:
    timings = {}
    for ob in stats:
...
def run_batch_cumulative_chart(stats) -> None:
    print("-----------")
    ...
    for cum in sorted(list(cummulatives)):
        ...
        su = plt.plot(
            cummulatives[cum], "-", linewidth=2, color=hexColour, label=keyString
        )
        charts.append(su)
